...
public class BowArrowItem extends GenericEntity {
	private int extraDmg;

	/***
	 * A constructor for the BowArrowItem class. Creates a sprite that can be
	 * collected by a player to increase arrow damage.
	 */

	public BowArrowItem(Vector spawnPos, int screenId, Renderer renderer) {
		// Fetch arrow image
		FileInputStream inputstream = null;
		try {
			inputstream = new FileInputStream(new File("res/graphics/bow.png"));
		} catch (FileNotFoundException e) {
			System.out.println("FileNotFoundException: bow.png not found");
		}
		...
		Vector size = new Vector(20.0, 50.0);
		...
		setEntitySprite(
				new Sprite(spawnPos, new Vector(0.0, 0.0), rand.nextInt(360), 0.0, new Vector(20.0, 50.0), imgs));
		setHitbox(hitbox);
			...
			public void onCollision(Sprite thisSprite, Sprite otherSprite) {
				...
				for (GenericEntity e : getEntities().get(screenId)) { // Find entity that collided
					if (e.getEntitySprite() == otherSprite) {
						...
						if (e instanceof Player) { // Player collided with BowArrowItem
							otherPlayerEntity = (Player) e;
							otherPlayerEntity.getSfx().PlayTrack(0);
							otherPlayerEntity.setExtraArrowDmg(otherPlayerEntity.getExtraArrowDmg() + getExtraDmg());
