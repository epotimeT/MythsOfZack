...
/**
 * <h1>GameStarter</h1> This class contains the methods needed to transfer from
 * the gui to the game
 */
public class GameStarter {

        /**
         * Boolean for whether sound and volume are on or off
         */
        boolean sound = true;
	...
	/**
	 * Creates a GameStarter object This also initialises the Gameloop object to
	 * seamlessly transfer into the game after finishing set-up
	 * 
	 * @param window - stage that the gui is displayed on
	 */
	public GameStarter(Stage window) {
		...
		gameRenderer.setShouldRenderHitboxes(false);

		InputHandler inputObject = new InputHandler();
		AnimationTimer gameLoop = new Gameloop(canvas, inputObject, gameRenderer, uiRenderer, window);
		gameLoop.start();
	...
	/**
	 * Loads the settings for the game ui
	 */
