...
import com.game.logic.InputCommand;
...
public class InputHandler {
			...
			public void handle(MouseEvent event) {
				if (player.isAlive()) {
					if (player.getEquippedWeapon() == weapon.SWORD) {
						...
						bowAttack(player, event, gameRenderer);
					} else if (player.getEquippedWeapon() == weapon.MAGIC) {
						magicAttack(player, event, gameRenderer);
					}
				}
			}
		});
	 ...
	 * @param event
	 */
	public void bowAttack(Player player, MouseEvent event, Renderer gameRenderer) {
		System.out.println("BOW ATTACK");
		if (player.getAtkCooldown() == 0.0 && player.getArrows() != 0) {
			System.out.println("triggering proj attack");
			GenericProjectile proj = RangedCommands.projAttack(player, new Vector(event.getX(), event.getY()),
					gameRenderer);
			// proj.removeFromRenderer(gameRenderer);

			Thread waiter = new Thread() {
				@Override
				public void run() {

					List<GenericProjectile> projList = player.getProjectileList();

					while (projList.contains(proj)) {
						projList = player.getProjectileList();
						try {
							Thread.sleep(100);
	 ...
	 * @param event
	 */
	public void magicAttack(Player player, MouseEvent event, Renderer gameRenderer) {
		if (player.getMagicPoints() >= 50 && player.getAtkCooldown() == 0.0) {
			Vector target = new Vector(event.getX(), event.getY());
			GenericProjectile proj = RangedCommands.projAttack(player, target, gameRenderer);
