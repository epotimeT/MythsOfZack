...
public class Area12 extends Areas {
	static Random rand = new Random();
	static final int screenId = 2;
	...
	public void obstacles(Vector canvas) {
		...
		// Left Door
		Door leftDoor = new Door(TransitionDirection.LEFT, screenId, canvas, 0);
		...
		spawnPos = new Vector(canvas.x() - size.x() - 125.0, 127.5);
		rock = new Rock(spawnPos, size, screenId);

		// Right bottom rock
		size = new Vector(150.0, 50.0);
		spawnPos = new Vector(canvas.x() - size.x() - 125.0, canvas.y() - size.y() - 127.5);
		rock = new Rock(spawnPos, size, screenId);
	}
	...
	public void items(Vector canvas, Renderer renderer) {
		...
		Vector[] spawnPointsItems = { new Vector((canvas.x() / 2) - (size.x() / 2), (canvas.y() / 2) - (size.y() / 2)), // Centre
																														// of
																														// screen
				new Vector(canvas.x() - size.x() - 125.0, (canvas.y() / 2) - (size.y() / 2)), // Left of screen, middle
				new Vector(218.75, (canvas.y() / 2) - (size.y() / 2) - 150.0), // Left of screen, top
				new Vector(218.75, (canvas.y() / 2) - (size.y() / 2) + 150.0) }; // Left of screen, bottom
