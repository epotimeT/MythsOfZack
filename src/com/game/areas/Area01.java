...
public class Area01 extends Areas {
	static Random rand = new Random();
	static final int screenId = 1;
	...
	public void obstacles(Vector canvas) {
		// Bottom Door
		Door bottomDoor = new Door(TransitionDirection.DOWN, screenId, canvas, 0);
	...
	public void monstersOffline(Vector canvas, Renderer renderer) {
		...
		for (int i = 4; i < spawnPointsMons.length; i++) {
			...
			if (monPro instanceof Slime) {
				...
				// monster.setSfx(new EntitySFX(monster, 1200f, 1));
			}
		 ...
		 * Vector((canvas.x()/2) - (size.x()/2) - 200.0, 75.0 - (size.y()/2)), // Top,
		 * middle, slightly left new Vector((canvas.x()/2) - (size.x()/2) + 200.0, 75.0
		 * - (size.y()/2)), // Top, middle, slightly right new Vector((canvas.x()/2) -
		 * (size.x()/2) - 100.0, (canvas.y()/2) - (size.y()/2)), // Centre of screen,
		 * slightly right new Vector((canvas.x()/2) - (size.x()/2) + 100.0,
		 * (canvas.y()/2) - (size.y()/2))}; // Centre of screen, slightly left
		 * 
		 * 
		 ...
		 * screenId); } else { // Never run monster = new
		 * Slime(spawnPointsMons[rand.nextInt(4)], screenId); }
		 * 
		 * <<<<<<< HEAD //monster.setSfx(new EntitySFX(monster,
		 * gameRenderer.getCanvas(), 1)); } }
		 * 
		 * public void items(Vector canvas) { ======= monster.setSfx(new
		 * EntitySFX(monster, gameRenderer.getCanvas(), 1)); }
		 */
	}
	...
	public void itemsOffline(Vector canvas, Renderer renderer) {
		Vector size = new Vector(50, 50);
		Vector[] spawnPointsItems = { new Vector((canvas.x() / 2) - (size.x() / 2), 75.0 - (size.y() / 2)), // Top of
																											// screen,
																											// middle
